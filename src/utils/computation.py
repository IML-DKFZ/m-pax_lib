...
class Leonhard(Node):
    ...
    def run_experiment(self, experiment_arguments, gpus, gpu_q, email, name):
        ...
        command = (
            "source .bash_profile; cd " + self.project_path + "; "
            "module load gcc/6.3.0; "
            "module load python_gpu/3.7.1; "
            "module load cuda/10.1.243; "
            "module load eth_proxy; "
            ...
            + " -W "
            + str(gpu_q)
            + ':00 -R "select[gpu_model0==TeslaV100_SXM2_32GB]" -R "rusage[mem=15000,ngpus_excl_p='
            + str(gpus)
            + ']" '
            "'python train.py " + experiment_arguments + "' "
        )
        ...
        subprocess.Popen(
            ["ssh", "%s" % "jcarvalho@euler.ethz.ch", command],
            shell=False,
...
class Euler(Node):
    ...
    def node_run_experiment(
        self, experiment_config, gpus, gpu_q, gpu_model, email, name, script_name
    ):
        """this is a non-blocking run"""
        print("Sending a job to Leonhard.")
        N = "-N " if email is True else ""
        ...
        command = (
            ...
            + " -W "
            + str(gpu_q)
            + ':00 -R "select[gpu_model0=='
            + gpu_model
            + ']" -R "rusage[mem=15000,ngpus_excl_p='
            + str(gpus)
            + ']" '
            "'python "
            + script_name
...
def run_experiment(
    ...
    for node in nodes:
        ...
        success = node.node_run_experiment(
            experiment_config, gpus, gpu_q, gpu_model, email, name, script_name
        )
        if success:
