...
class ISICDataModule(pl.LightningDataModule):
    ...
    def prepare_data(self):
        if not os.path.exists(os.path.join(self.data_dir, "ISIC/ISIC_2019_Training_Input")):
    ...
    def setup(self):
        ...
        data = ISICDataset(img_path=self.data_dir + "/ISIC/ISIC_2019_Training_Input", 
            transform=transform_img, 
            csv_path=self.data_dir + "/ISIC/labels.csv")

        self.train_enc, self.val_enc, self.train_head, self.val_head, self.test = random_split(
            data, [20000, 3000, 250, 81, 2000], 
            generator=torch.Generator().manual_seed(self.seed))


        #### Computing and distributing weights for weighted sampling ####
        weights_train_head = torch.DoubleTensor(make_weights_for_balanced_classes(self.train_head, 9))
        weights_val_head = torch.DoubleTensor(make_weights_for_balanced_classes(self.val_head, 9))
        weights_test = torch.DoubleTensor(make_weights_for_balanced_classes(self.test, 9))

        self.sampler_train_head = torch.utils.data.sampler.WeightedRandomSampler(weights_train_head, len(weights_train_head))
        self.sampler_val_head = torch.utils.data.sampler.WeightedRandomSampler(weights_val_head, len(weights_val_head))
        self.sampler_test = torch.utils.data.sampler.WeightedRandomSampler(weights_test, len(weights_test))
    ...
    def train_dataloader_head(self):
        return DataLoader(self.train_head, batch_size=self.batch_size, num_workers=self.num_workers, sampler=self.sampler_train_head, pin_memory = self.pin_memory)
    ...
    def val_dataloader_head(self):
        return DataLoader(self.val_head, batch_size=self.batch_size, num_workers=self.num_workers, sampler=self.sampler_val_head, pin_memory = self.pin_memory)
